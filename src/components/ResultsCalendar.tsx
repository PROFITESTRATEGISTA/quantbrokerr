...
import { Calendar, TrendingUp, TrendingDown, BarChart3, Filter, MessageCircle, Plus, AlertCircle, Edit3, Save, X, Check, Shield, Settings, CheckCircle } from 'lucide-react';
        ...
        {isAdmin && (
          <div className="mb-12 mt-16">
            ...
            <div className="bg-gradient-to-br from-red-900/20 via-slate-800/40 to-orange-900/20 rounded-xl p-6 border border-red-500/30">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
                  ...
                  <div className="bg-slate-800/50 p-4 rounded-lg border border-slate-600">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="text-xs text-gray-400">Limite Perda Diária</span>
                    </div>
                    {editingRisk ? (
                      <div className="flex items-center gap-2">
                        <input
                          ...
                          className="w-16 px-2 py-1 text-sm bg-slate-700 border border-slate-500 rounded text-white focus:border-red-500 focus:outline-none"
                        />
                        <span className="text-red-400 font-bold">%</span>
                      </div>
                    ) : (
                      <div className="text-xl font-bold text-red-400">
                        -{riskSettings.dailyLossLimit.toFixed(1)}%
                      </div>
                    )}
                  </div>
                  ...
                  <div className="bg-slate-800/50 p-4 rounded-lg border border-slate-600">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <span className="text-xs text-gray-400">Limite Perda Mensal</span>
                    </div>
                    {editingRisk ? (
                      <div className="flex items-center gap-2">
                        <input
                          type="number"
                          step="0.1"
                          value={riskSettings.monthlyLossLimit}
                          onChange={(e) => setRiskSettings({...riskSettings, monthlyLossLimit: parseFloat(e.target.value) || 0})}
                          className="w-16 px-2 py-1 text-sm bg-slate-700 border border-slate-500 rounded text-white focus:border-orange-500 focus:outline-none"
                        />
                    ...
                    {editingRisk ? (
                      <div className="flex items-center gap-2">
                        <input
                          ...
                          onChange={(e) => setRiskSettings({...riskSettings, stopLossPercentage: parseFloat(e.target.value) || 0})}
                          className="w-16 px-2 py-1 text-sm bg-slate-700 border border-slate-500 rounded text-white focus:border-purple-500 focus:outline-none"
                        />
                        <span className="text-purple-400 font-bold">%</span>
                      </div>
                    ) : (
                      <div className="text-xl font-bold text-purple-400">
                        -{riskSettings.stopLossPercentage.toFixed(1)}%
                      </div>
                    )}
                  </div>
                </div>
                  ...
                  <div className="flex items-center gap-4">
                    <div className="flex items-center gap-2">
                      <input
                        type="checkbox"
                        id="riskActive"
                        checked={riskSettings.isActive}
                        onChange={(e) => setRiskSettings({...riskSettings, isActive: e.target.checked})}
                        disabled={!editingRisk}
                        className="h-4 w-4 text-red-600 focus:ring-red-500 border-gray-300 rounded"
                      />
                      <label htmlFor="riskActive" className="text-sm text-gray-300">
                    ...
                    <div className={`px-3 py-1 rounded-full text-xs font-medium ${
                      ...
                      {riskSettings.isActive ? 'ATIVO' : 'INATIVO'}
                    </div>
                  </div>
                    ...
                    {editingRisk ? (
                      <>
                        ...
                        <button
                          onClick={() => {
                            setEditingRisk(false);
                            loadRiskSettings(); // Reload original settings
                          }}
                          className="flex items-center gap-2 bg-gray-600 hover:bg-gray-700 px-4 py-2 rounded-lg transition-colors text-sm font-medium"
                        >
                          <X className="w-4 h-4" />
                          Cancelar
                        </button>
                ...
                {/* Risk Explanation */}
                <div className="mt-6 bg-slate-800/30 rounded-lg p-4 border border-slate-600/50">
                  <h4 className="font-semibold text-white mb-3 flex items-center gap-2">
                    <Shield className="w-4 h-4 text-blue-400" />
                    Como Funciona o Controle de Risco
                  </h4>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-gray-300">
                    <div>
                      <h5 className="font-medium text-red-300 mb-2">Limite de Perda Diária</h5>
                      <p>Quando a perda diária atinge {riskSettings.dailyLossLimit}%, todas as operações são pausadas até o próximo dia útil.</p>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <p>Stop loss automático em {riskSettings.stopLossPercentage}% por operação individual.</p>
                    </div>
                  </div>
                </div>
              </div>
